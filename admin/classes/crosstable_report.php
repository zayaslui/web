...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("promociones" == $this->xml_array['tables'][0])
			$isDB = true;
		if("monedas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("detalles" == $this->xml_array['tables'][0])
			$isDB = true;
		if("sliders" == $this->xml_array['tables'][0])
			$isDB = true;
		if("imagenes_promociones" == $this->xml_array['tables'][0])
			$isDB = true;
		if("destinos" == $this->xml_array['tables'][0])
			$isDB = true;
		if("empresas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("usuarios" == $this->xml_array['tables'][0])
			$isDB = true;
		if("zonas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("galerias" == $this->xml_array['tables'][0])
			$isDB = true;
		if("menu_servicios" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
	}
