...
$layout = new TLayout("export2", "OfficeCoral", "MobileCoral");
$layout->version = 2;
...
$page_layouts["imagenes_promociones_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["imagen_promocion"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["imagen_promocion"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
