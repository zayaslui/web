...
function checkTableName($shortTName, $type=false)
{
	...
	if ("promociones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("monedas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("detalles" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("sliders" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("imagenes_promociones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("destinos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("empresas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("usuarios" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("zonas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("galerias" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("menu_servicios" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("promociones");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="promociones";
	}
	$strPerm = GetUserPermissions("monedas");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="monedas";
	}
	$strPerm = GetUserPermissions("detalles");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="detalles";
	}
	$strPerm = GetUserPermissions("sliders");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="sliders";
	}
	$strPerm = GetUserPermissions("imagenes_promociones");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="imagenes_promociones";
	}
	$strPerm = GetUserPermissions("destinos");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="destinos";
	}
	$strPerm = GetUserPermissions("empresas");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="empresas";
	}
	...
	$strPerm = GetUserPermissions("zonas");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="zonas";
	}
	$strPerm = GetUserPermissions("galerias");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="galerias";
	}
	$strPerm = GetUserPermissions("menu_servicios");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="menu_servicios";
	}
